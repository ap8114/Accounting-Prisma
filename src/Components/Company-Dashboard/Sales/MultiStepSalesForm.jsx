...
const MultiStepSalesForm = ({
  ...
  const pdfRef = useRef();
  const company_id = GetCompanyId();
  // --- Form Data State - Updated to match new API schema ---
  const [formData, setFormData] = useState(() => {
    const initialFormData = {
      // Company Info
      companyInfo: {
  ...
  const handleSaveNext = () => {
    ...
    setKey((prev) => {
      ...
      if (prev === "deliveryChallan") return "invoice";
      if (prev === "invoice") {
        const totalInvoiceAmount = calculateTotalWithTaxAndDiscount(formData.items);
        setFormData(prevData => ({
          ...prevData,
          steps: {
            ...prevData.steps,
      ...
      >
        ...
        {key === "payment" && (
          <div style={{ marginBottom: "15px" }}>
            ...
            <p>
              <strong>Amount Paid:</strong> $
              {parseFloat(stepData.amount_received || 0).toFixed(2)}
            </p>
            <p>
